...
func GetChtRoot(db fdgdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func StoreChtRoot(db fdgdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	disablePruning       bool
	diskdb, trieTable    fdgdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db fdgdb.Database, odr OdrBackend, size, confirms uint64, disablePruning bool) *core.ChainIndexer {
...
func (c *ChtIndexerBackend) Prune(threshold uint64) error {
	...
	for {
		...
		if batch.ValueSize() > fdgdb.IdealBatchSize {
...
func GetBloomTrieRoot(db fdgdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db fdgdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db fdgdb.Database, odr OdrBackend, parentSize, size uint64, disablePruning bool) *core.ChainIndexer {
