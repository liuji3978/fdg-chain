...
var (
	...
	debExecutables = []debExecutable{
		...
		{
			BinaryName:  "gfdg",
			Description: "FDG CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "FDG private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "FDG account management tool.",
		},
	}
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 13 {
			...
			log.Println("go-fdg requires at least Go version 1.13 and cannot")
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-fdg sources.
...
type debMetadata struct {
	...
	// go-fdg version being built. Note that this
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/liuji3978/fdg-chain/mobile"))
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/liuji3978/fdg-chain/mobile")
