...
func (db *Database) NewBatch() fdgdb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewIterator(prefix []byte, start []byte) fdgdb.Iterator {
...
func (b *batch) Replay(w fdgdb.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  fdgdb.KeyValueWriter
	failure error
}
