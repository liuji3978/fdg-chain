...
func InitDatabaseFromFreezer(db fdgdb.Database) {
...
func iterateTransactions(db fdgdb.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db fdgdb.Database, from uint64, to uint64) {
...
func UnindexTransactions(db fdgdb.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
